...
mutect_out=~/dataLocal/results/MuTect2/$base2.vcf
filter_out=~/dataLocal/results/MuTect2/${base2}_filtered.vcf

echo "Processing files $b1 and $b2"
# Run MuTect2 and move output to the appropriate folder
java -jar /home/student/RNAseq/gatk-4.1.2.0/gatk-package-4.1.2.0-local.jar Mutect2 -R $genome -I $b1 -I $b2 --normal normal --disable-read-filter MateOnSameContigOrNoMappedMateReadFilter --dont-use-soft-clipped-bases -O $mutect_out
...
~/RNAseq/cufflinks-2.2.1.Linux_x86_64/cuffdiff $gtf $b1 $b2 -L tumor,normal -o ~/dataLocal/results/Cufflinks
